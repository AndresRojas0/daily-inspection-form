...
import { Plus, Trash2, Clock, MapPin, Users, CheckCircle, AlertCircle, ArrowLeft, Save } from "lucide-react"
import { updateDailyInspectionForm } from "@/lib/actions"
import Link from "next/link"
import { useRouter } from "next/navigation"
import type { DailyInspectionFormWithChecks } from "@/lib/database"
...
interface EditFormClientProps {
  form: DailyInspectionFormWithChecks
}

export function EditFormClient({ form: initialForm }: EditFormClientProps) {
  const router = useRouter()

  // Convert database form to client form format
  const [form, setForm] = useState<DailyInspectionForm>({
    ...
    serviceChecks: initialForm.service_checks.map((check) => ({
      ...
      lineOrRouteNumber: check.line_or_route_number,
      driverName: check.driver_name,
      serviceCode: check.service_code,
      fleetCoachNumber: check.fleet_coach_number,
      exactHourOfArrival: check.exact_hour_of_arrival,
      gpsStatus: {
        minutes: check.gps_minutes,
        status: check.gps_status,
      },
      passengersOnBoard: check.passengers_on_board,
      passesUsed: check.passes_used,
      addressOfStop: check.address_of_stop,
      observations: check.observations || "",
      nonCompliance: check.non_compliance,
    })),
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateDailyInspectionForm(initialForm.id, form)
      setSubmitResult(result)

      if (result.success) {
        // Redirect back to form details after successful update
        setTimeout(() => {
          router.push(`/dashboard/${initialForm.id}`)
        }, 1500)
      }
    } catch (error) {
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href={`/dashboard/${initialForm.id}`}>
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Edit Inspection Form</h1>
              <p className="text-gray-600">Form ID: {initialForm.id}</p>
            </div>
          </div>
        </div>
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>Edit your daily service inspection</CardDescription>
          </CardHeader>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Edit details for each service check</CardDescription>
              </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <Button onClick={handleSubmit} className="w-full" size="lg" disabled={!isFormValid || isSubmitting}>
              ...
              {isSubmitting ? "Updating..." : "Update Daily Inspection Form"}
            </Button>
